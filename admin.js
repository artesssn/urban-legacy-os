...
async function loadCloudState() {
  ...
  state = {
    ...
    references: referencesResult.error ? state.references || [] : referencesResult.data.map(fromReferenceRow)
  };
...
function renderSelectors() {
  ...
  $("reference-customer").innerHTML = state.customers
    .map((customer) => `<option value="${customer.id}">${customer.name}</option>`)
    .join("") || "<option value=''>Cliente avulso</option>";
  syncSalePrice();
}
...
async function saveReference(event) {
  ...
  if (cloudEnabled) {
    ...
    if (error) return toast("Referência salva localmente. Rode o schema para salvar na nuvem.");
  } else {
    saveState();
  }
...
function renderReferences() {
  ...
  $("reference-count").textContent = `${references.length} referências`;
  $("reference-list").innerHTML = references.map((reference) => `
    <article class="customer-item">
      <div>
        <p class="item-title">${reference.customerName} • ${reference.type}</p>
        <p class="item-sub">
          ${reference.style || "Sem estilo"} • ${reference.colors || "Sem cores"} • ${reference.budget ? money.format(reference.budget) : "Sem orçamento"}
        </p>
        <p class="item-sub">
          Superior: ${reference.topSize || "-"} • Inferior: ${reference.bottomSize || "-"} • Calçado: ${reference.shoeSize || "-"}
        </p>
        ${reference.notes ? `<p class="item-sub">${reference.notes}</p>` : ""}
        ${reference.link ? `<a class="item-sub" href="${reference.link}" target="_blank" rel="noreferrer">${reference.link}</a>` : ""}
      </div>
      <span class="pill">${reference.type}</span>
    </article>
  `).join("") || `<p class="item-sub">Nenhuma referência cadastrada.</p>`;
}
